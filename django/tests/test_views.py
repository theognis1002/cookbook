...
class TestMyCreateView:
    ...
    def test_post(self):
        ...
        data = {"name": "Hans", "other_model": mixer.blend("myapp.MyOtherModel").pk}
...
class TestMyUpdateView:
    def test_authentication(self):
        ...
        req = RequestFactory().get(
            reverse("myapp:myupdateview", kwargs={"pk": my_model.pk})
        )
        req.user = AnonymousUser()
    ...
    def test_post(self):
        ...
        data = {"name": "Hans", "other_model": my_model.other_model.pk}
        req = RequestFactory().post(
            reverse("myapp:myupdateview", kwargs={"pk": my_model.pk}), data=data
        )
        req.user = mixer.blend(User)
    ...
    def test_invalid_data(self):
        ...
        ), "should not redirect to success url, when data is invalid"
        assert my_model.name == "Dieter"
        my_model.refresh_from_db()
    ...
    def test_valid_form_data(self):
        ...
        req = RequestFactory().post(
            reverse("myapp:myupdateview", kwargs={"pk": my_model.pk}), data=form.data
        )
        req.user = mixer.blend(User)
